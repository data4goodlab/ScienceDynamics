...
from configs import logger, AuthorNotFound
...
class AuthorsFetcher(object):
    ...
    def get_author_data(self, author_id=None, author_name=None):
        """
        Returns suthors data as dict there are two types of data 'features' that include the author general features,
    ...
    def _get_author_by_id(self, author_id):
        ...
        try:
            ...
            logger.warning('Failed to fetch author %s features ' % author_id)
            raise AuthorNotFound()
        # converting the year keys back to int from string
        for k, d in j.iteritems():
            if "by Year" in k and d is not None:
                j[k] = {int(y): v for y, v in d.iteritems()}
        return j
    ...
    def _get_author_by_name(self, author_name):
        ...
        try:
            logger.debug("Fetching first author matching name %s" % author_name)
            j = self._authors_features_collection.find({"Author name": author_name}).next()

        except StopIteration, e:
            logger.warning('Failed to fetch author %s features ' % author_name)
        # converting the year keys back to int from string
        for k, d in j.iteritems():
            if "by Year" in k:
                j[k] = {int(y): v for y, v in d.iteritems()}
