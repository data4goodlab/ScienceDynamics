...
def cross_domain_figure_data():
    ...
    for i in [0,1]:
        ...
        g = sf.groupby('Paper publish year',
                       {"Total Number of Papers": agg.COUNT(), "L%s Fields Number List" % i: agg.CONCAT('L%s Fields Number' % i)})
...
def total_citation_after_years_years(y=3):
    ...
    g['papers_with_zero_after_%s_no_self_citations' % y] = g['total_citations_list_no_self_citations'].apply(lambda l: len([i for i in l if i == 0]))
    g['zero_citation_percentage_%s' %y] = g.apply(lambda r: r['papers_with_zero_after_%s' % y]/float(r['total_papers']))
    g['zero_citation_percentage_no_self_%s' % y] = g.apply(lambda r: r['papers_with_zero_after_%s_no_self_citations' % y]/float(r['total_papers']))
    x = g.sort('Paper publish year', ascending=False)
    x['Paper publish year','zero_citation_percentage_%s' %y, 'zero_citation_percentage_no_self_%s' % y].save('/home/michael/article_results/papers_no_citations_after%s.csv' % y, format="csv")

def citations_figure_data(after_years=(1,3,5,10)):
    sf_list = [gl.SFrame.read_csv('/home/michael/article_results/papers_no_citations_after%s.csv' % y) for y in after_years]
    sf = sf_list[0]
...
def get_authors_per_paper_number_figure_data():
    ...
    g['List of Author ID'] = g['List of Author ID'].apply(lambda l: list(set(l))) # double checking no authors count twice
    ...
    z = g3['Paper publish year','Average Authors Number with At least 5 Ref','Median Authors Number with At least 5 Ref']
    y = y.join(z)
    y = y.sort('Paper publish year', ascending=False)
...
def get_authors_publications_by_year_data():
    ...
    g3['normalized_publications_dict'] = g3.apply( lambda r: {(k-r['start_year']):len(v) for k,v in r['all_publications_dict'].iteritems()})
    ...
    for i in range(0, 51, 1):
        print i
        g4['%s' % i]= g4.apply(lambda r: np.average(get_number_of_publications_list_after_years(i, r['publications_dicts_list'])) if (r['start_year'] + i) <= 2014 else None )
        g4.__materialize__()
    ...
    g3['Publications Yearly Rate'] = g3.apply(lambda r: get_average_number_of_publication_a_year(r['start_year'], r['end_year'], sum(r['normalized_publications_dict'].values())))
    g5 = g3.groupby('start_year', {'Authors Yearly Publications Rate List': agg.CONCAT('Publications Yearly Rate')})
    g5['Average Authors Yearly Publication Rate'] = g5['Authors Yearly Publications Rate List'].apply(lambda l: np.average(l))
    g5['Median Authors Yearly Publication Rate'] = g5['Authors Yearly Publications Rate List'].apply(lambda l: np.median(l))
...
def get_gender_top_papers_rank_over_time():
    ...
    g['All Authors Gender Stats'] = g['Paper publish year', 'Papers Ids'].apply(lambda r: dict(PaperCollections(papers_ids=r['Papers Ids']).authors_gender_stats(r['Paper publish year'])))
    g['First Authors Gender Stats'] = g['Paper publish year', 'Papers Ids'].apply(lambda r: dict(PaperCollections(papers_ids=r['Papers Ids']).first_authors_gender_stats(r['Paper publish year'])))
    g['Last Authors Gender Stats'] = g['Paper publish year', 'Papers Ids'].apply(lambda r: dict(PaperCollections(papers_ids=r['Papers Ids']).last_authors_gender_stats(r['Paper publish year'])))
    ...
    g2['Paper publish year', 'All Authors Female Percentage', 'First Authors Female Percentage', 'Last Authors Female Percentage'].save('/home/michael/article_results/gender_stat.csv')
...
def journal_returning_authors_get_data(outdir="/home/michael/article_results/selected_journals_features"):
    ...
    for j in journals_list:
        ...
        json.dump(features_dict, file("%s/%s.json" % (outdir, j), "w"))
