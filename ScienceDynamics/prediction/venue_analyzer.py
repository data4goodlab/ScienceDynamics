from configs import *
import os
from utils import get_column_count_sframe, filter_sframe_by_func
import itertools
...
class VenueAnalyzer(object):
    ...
    def _get_all_papers_sframe(self):
        ...
        """
        if self.venue_type == VenueType.journal:
            return gl.load_sframe("%s/%s.sframe" % (JOURNALS_PAPERS_SFRAMES_DIR, self._venue_id))
        elif self.venue_type == VenueType.conference:
            return gl.load_sframe("%s/%s.sframe" % (CONFERENCES_PAPERS_SFRAMES_DIR, self._venue_id))
    ...
    def get_yearly_percentage_of_papers_with_new_authors_dict(self, authors_seq_list):
        return {i: self.get_percentage_papers_by_new_authors(i, authors_seq_list) for i in range(self.min_year + 1, self.max_year + 1)}
    ...
    def average_and_median_academic_age(self, year, authors_seq_list):
        ...
        academic_birthyears_list = [year - self.authors_academic_birthyear_dict[a] for a in authors_list if a in self.authors_academic_birthyear_dict]
    ...
    def get_average_and_median_academic_age_dict(self, authors_seq_list=None):
        return {i: self.average_and_median_academic_age(i, authors_seq_list) for i in range(self.min_year + 1, self.max_year + 1)}
    ...
    def update_venue_stats(self):
        j = json.load(file("/data/json/journals/%s (%s).json" % (self.name, self.venue_id), "r"))
        logger.info("update features of %s" % self.name)
        ...
        json.dump(j, file("/data/json/journals/%s (%s).json" % (self.name, self.venue_id), "w"))
    ...
    def get_venue_authors_links_timeseries(self):
        ...
        links_sf = links_sf[links_sf["src_id"] <> links_sf["dst_id"]]  # because this is a direct network we keep for each link both (u,v) and (v,u)
        return gl.TimeSeries(links_sf,index="datetime")

    def create_timeseries(self, outpath):
        v_ts = self.get_venue_authors_timeseries()
        v_ts.save("%s/%s.vertices.timeseries" % (outpath, self._venue_id))
        i_ts = self.get_venue_authors_links_timeseries()
        i_ts.save("%s/%s.interactions.timeseries" % (outpath, self._venue_id))
    ...
    def fields_of_study_sframe(self):
        if os.path.isdir(FIELDS_OF_STUDY_SFRAME):
            return gl.load_sframe(FIELDS_OF_STUDY_SFRAME)
        return gl.load_sframe(FIELDS_OF_STUDY_S3_SFRAME)

    @property
    def fields_of_study_hierarchy_sframe(self):
        if os.path.isdir(FIELDS_OF_STUDY_HIERARCHY_SFRAME):
            return gl.load_sframe(FIELDS_OF_STUDY_HIERARCHY_SFRAME)
        return gl.load_sframe(FIELDS_OF_STUDY_HIERARCHY_S3_SFRAME)

    @property
    def papers_sframe(self):
        if os.path.isdir(PAPERS_SFRAME):
            return gl.load_sframe(PAPERS_SFRAME)
        return gl.load_sframe(PAPERS_S3_SFRAME)

    @property
    def extended_papers_sframe(self):
        if os.path.isdir(EXTENDED_PAPERS_SFRAME):
            return gl.load_sframe(EXTENDED_PAPERS_SFRAME)
        return gl.load_sframe(EXTENDED_PAPERS_S3_SFRAME)

    @property
    def paper_author_affiliations_sframe(self):
        if os.path.isdir(PAPER_AUTHOR_AFFILIATIONS_SFRAME):
            return gl.load_sframe(PAPER_AUTHOR_AFFILIATIONS_SFRAME)
        return gl.load_sframe(PAPER_AUTHOR_AFFILIATIONS_S3_SFRAME)

    @property
    def paper_keywords_sframe(self):
        if os.path.isdir(PAPER_KEYWORDS_SFRAME):
            return gl.load_sframe(PAPER_KEYWORDS_SFRAME)
        return gl.load_sframe(PAPER_KEYWORDS_S3_SFRAME)

    @property
    def paper_references_sframe(self):
        if os.path.isdir(PAPER_REFERENCES_SFRAME):
            return gl.load_sframe(PAPER_REFERENCES_SFRAME)
        return gl.load_sframe(PAPER_REFERENCES_S3_SFRAME)
    ...
    def coauthors_links_sframe(self):
        ...
        if os.path.isdir(CO_AUTHORSHIP_LINK_SFRAME):
            self._co_authors_links = gl.load_sframe(CO_AUTHORSHIP_LINK_SFRAME)
        else:
            self._co_authors_links = gl.load_sframe(CO_AUTHORSHIP_LINK_S3_SFRAME)
        return self._co_authors_links

    @property
    def l3_field_papers_sframe(self):
        if os.path.isdir(L3_FIELD_PAPERS_LIST_SFRAME):
            return gl.load_sframe(L3_FIELD_PAPERS_LIST_SFRAME)
        return gl.load_sframe(L3_FIELD_PAPERS_LIST_S3_SFRAME)

    @property
    def journals_sframe(self):
        if os.path.isdir(JOURNALS_SFRAME):
            return gl.load_sframe(JOURNALS_SFRAME)
        return gl.load_sframe(JOURNALS_S3_SFRAME)

    @property
    def confrences_sframe(self):
        if os.path.isdir(CONFRENCES_SFRAME):
            return gl.load_sframe(CONFRENCES_SFRAME)
        return gl.load_sframe(CONFRENCES_S3_SFRAME)
...
def get_all_journals_features():
    academic_birthyear_dict = cPickle.load(file(JOURNAL_AUTHORS_ACADEMIC_BIRTHYEAR_PKL, "rb"))
    for i in os.listdir("/data/sframes/journals"):
        logger.info("Analyzing %s " % i )
        try:
            jid = i.split(".sframe")[0]
            va = VenueAnalyzer(jid, VenueType.journal, academic_birthyear_dict)
            j = va.get_venue_stats()
            json.dump(j, file("/data/json/journals/%s (%s).json" % (va.name, va.venue_id), "w"))
        except Exception, e:
            print e.message
            logger.error(e.message)
...
def create_all_timeseries():
    for i in os.listdir("/data/sframes/journals"):
        jid = i.split(".sframe")[0]
        print "Createing %s timeseries" % jid
        va = VenueAnalyzer(jid, VenueType.journal, None)
        p = "/data/timeseries/journals/%s" % jid
        if os.path.isdir(p):
            continue
        os.mkdir(p)
